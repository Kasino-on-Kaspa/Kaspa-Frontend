...
import { Route } from "@/routes/referral";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function ReferralPage() {
  ...
  const { referralCode: prefilledReferralCode } = Route.useSearch();
  ...
  // Add useEffect to handle prefilled referral code
  useEffect(() => {
    if (prefilledReferralCode && !userData?.referredBy) {
      setReferralCode(prefilledReferralCode);
      setShowConfirmDialog(true);
    }
  }, [prefilledReferralCode, userData?.referredBy]);
  ...
  const handleConfirmReferral = async () => {
    ...
    try {
      await updateReferredBy(referralCode);
      await refetch();
      toast.success("Successfully updated referral code");
    } catch (error) {
      toast.error("Failed to update referral code");
    } finally {
      setIsLoading(false);
  ...
  return (
    <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                },
                {
                  ...
                  description:
                    "24/7 support from our affiliate management team",
                },
                {
                      ...
                      <Button
                        ...
                        onClick={() => {
                          navigator.clipboard.writeText(
                            `${window.location.origin}/referral?referralCode=${userData?.referralCode}`,
                          );
            ...
            <div className="container mx-auto px-4">
              ...
              >
                <h2 className="text-3xl md:text-4xl font-bold mb-6">
                  Ready to Start{" "}
                  <span className="text-[#6fc7ba]">Earning?</span>
                </h2>
                <p className="text-white/70">
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent className="bg-[#1a1718] border border-white/ p-5">
          <DialogHeader>
            <DialogTitle className="text-white font-Onest">
              Confirm Referral
            </DialogTitle>
            <DialogDescription className="text-white/70 font-Onest">
              Do you want to be referred by {referralCode}?
            </DialogDescription>
          </DialogHeader>
